/**
 * Renders a sanitized Toolkit post body. Unlike lesson content, toolkit templates
 * are meant to be read AND copied, so there is no copy guard here. The HTML is
 * sanitized on save with the same allowlist as lessons.
 */
const TOOLKIT_CLASS =
  "text-slate-700 " +
  ...
  "[&_blockquote]:my-4 [&_blockquote]:rounded-md [&_blockquote]:border-l-2 [&_blockquote]:border-gold-500 [&_blockquote]:bg-neutral-50 [&_blockquote]:px-4 [&_blockquote]:py-2 [&_blockquote]:text-slate-700 " +
...
export function ToolkitBody({ html }: { html: string }) {
  return <div className={TOOLKIT_CLASS} dangerouslySetInnerHTML={{ __html: html }} />;
}
