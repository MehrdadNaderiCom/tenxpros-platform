...
// per-function rates or the caps. Delivery uses the single configured rate.
const sumB2c = CFG.qualifiedOriginationB2cBp + CFG.closingB2cBp + CFG.deliveryPercentBp;
const paidB2c = Math.min(sumB2c, CFG.capB2cBp);
const sumB2b = CFG.strongOriginationB2bBp + CFG.closingB2bBp + CFG.deliveryPercentBp;
const paidB2b = Math.min(sumB2b, CFG.capB2bBp);
// A B2C example that intentionally EXCEEDS the cap (a strong origination stacked with
// closing and delivery) so the scale-down to fit is visible, config-sourced so it never drifts.
const strongSumB2c = CFG.strongOriginationB2cBp + CFG.closingB2cBp + CFG.deliveryPercentBp;
// A worked origination-override figure: the override pays a config share of the
// opener's rate. On a strong B2B opening this matches the engine's rounding.
const overrideOnStrongB2b = Math.round((CFG.strongOriginationB2bBp * CFG.overrideShareBp) / 10000);
...
<ul>
  ...
  <li><strong>The paid functions:</strong> basic introduction, origination, closing, and delivery or coaching. A basic introduction is a light referral, you introduce a relevant contact, and it pays only if that contact becomes a closed, paid deal, but it does not register or protect the account for you. Origination is bringing and qualifying the opportunity and registering the account so your role is protected, closing is getting it to a paid decision, and delivery or coaching is doing approved work inside the engagement. On a B2B account you originated, a later renewal can also pay you an origination override, described below.</li>
  ...
  <li><strong>Growth bonus and focus bonus:</strong> extra earnings a qualifying tier two or tier three partner can become eligible for. The growth bonus rewards bringing in several distinct, genuinely new B2B organizations within the rolling window, counted by company domain so two deals on the same company count once, and a deal with no recorded domain never counts toward it. The growth bonus is paid inside the per deal cap. The Tier 3 focus bonus is the single exception that can lift an eligible focus deal above the base cap, up to the ${pct(CFG.tier3FocusHardCeilingBp)} focus ceiling, and it is the only thing that ever pays above the base cap.</li>
...
<p>Commission is earned by function and calculated on net receipts actually received and cleared. The unit of sale is the seat, meaning one enrolled professional, sold either as a business to consumer charter (an individual enrolls) or a business to business engagement (an organization buys seats). Stronger origination and closing pay more than a basic introduction, delivery or coaching pays a single company-set rate when the company engages a partner to help it scale, and there is a cap per deal. The exact figures live in the official terms. The logic to remember: you are paid on cleared money, for the function you performed, up to a cap, never simply because a conversation happened.</p><div class="callout callout-info"><p><strong>The current rates, by function.</strong> These are the current published rates. They can change, so always treat your Panel and the current published terms as the source of truth, not a number you memorized.</p></div><ul><li>Basic Introduction: ${pct(CFG.basicIntroductionBp)} on B2C and B2B. You actively introduce and explain us to someone you have a genuine, pre-existing warm relationship with (attested, with a note naming the person and the relationship), and then step away: zero meetings and no follow-up. A genuinely valuable introduction is confirmed and rewarded even though you introduce and step aside. It pays only when that introduced contact becomes a closed, paid deal, and an introduction does not register or protect the account for you, which is why origination pays more.</li><li>Qualified Origination: ${pct(CFG.qualifiedOriginationB2cBp)} on B2C, ${pct(CFG.qualifiedOriginationB2bBp)} on B2B. Origination goes beyond an introduction: you attend the meetings and take on the follow-up, actively advancing the account.</li><li>Strong Origination: ${pct(CFG.strongOriginationB2cBp)} on B2C, ${pct(CFG.strongOriginationB2bBp)} on B2B.</li><li>Closing: ${pct(CFG.closingB2cBp)} on B2C, ${pct(CFG.closingB2bBp)} on B2B, paid once the deal carries a recorded signed date. Closing means you drive the deal to a signed, started contract yourself: our team gives at most one online meeting of under one hour, and you carry everything else through payment cleared, contract signed, and the engagement started.</li><li>Delivery or Coaching: ${pct(CFG.deliveryPercentBp)} of Net Receipts, a single rate the company sets. Delivery is normally the company's own; this line is paid only when the company engages you to help it scale, once the deal carries a recorded delivered date. You never set this rate, and a fixed fee is possible only as a company exception recorded with a reason.</li></ul><p><strong>Qualified or strong, and who decides.</strong> You never set your own rate or decide whether an origination counts as qualified or strong. The company derives it objectively from the recorded facts of the deal, so you are not guessing and nothing is a matter of opinion. The deciding fact is the SEAT COUNT, counted on paid and collected seats. On a B2C Charter, seats alone decide: ${CFG.strongSeatThresholdB2c} paid seats or more is a Strong Origination (an individual with a real community who sells volume, and an individual has no domain, so the domain plays no role on B2C); below that it is Qualified. On a B2B Engagement two things must both be true: the company's domain is genuinely New or Dormant, and the deal reaches ${CFG.strongSeatThresholdB2b} paid seats or more. A company's newness is read from its domain: <strong>New</strong> means the domain has never appeared on a closed deal with us, <strong>Dormant</strong> means it has but its most recent activity is more than ${CFG.originationWindowMonths} months ago, and <strong>Existing</strong> means there has been activity within the last ${CFG.originationWindowMonths} months. Opening a new unit of an Existing company is a Qualified Origination. Here is the sentence that keeps you from ever being surprised: if the company is New or Dormant but the deal is below the seat threshold, it keeps its new-company standing but is paid the Qualified rate, not the Strong rate. A deal with no paid seats recorded yet is always Qualified, never Strong, and on B2B a deal with no recorded domain cannot claim the Strong rate at all.</p><p><strong>The cap, and how functions add up.</strong> Total partner compensation on any one deal is capped: ${pct(CFG.capB2cBp)} of Net Receipts on B2C and ${pct(CFG.capB2bBp)} on B2B, and an eligible Tier 3 focus account can rise gradually to ${pct(CFG.tier3FocusHardCeilingBp)}. When you perform more than one function on the same deal, the rates add up, and the total is then clamped to that cap. Two plain examples. First, on a B2C deal where you qualified the opportunity (${pct(CFG.qualifiedOriginationB2cBp)}), closed it (${pct(CFG.closingB2cBp)}), and delivered at ${pct(CFG.deliveryPercentBp)}, the rates add to ${pct(sumB2c)}. The B2C cap is ${pct(CFG.capB2cBp)}, so you are paid ${pct(paidB2c)} of Net Receipts, and any total above the cap is clamped to it. Second, on a B2B deal where you strongly originated (${pct(CFG.strongOriginationB2bBp)}), closed (${pct(CFG.closingB2bBp)}), and delivered at ${pct(CFG.deliveryPercentBp)}, the rates add to ${pct(sumB2b)}. The B2B cap is ${pct(CFG.capB2bBp)}, so you are paid ${pct(paidB2b)}. For an ordinary deal this cap is an absolute ceiling that no extra function and no growth bonus can push past; the one exception is a Tier 3 focus deal, where the focus bonus can lift the ceiling gradually to ${pct(CFG.tier3FocusHardCeilingBp)}, explained next. When a stacked total does go over the cap, the percentage lines are scaled down in proportion to fit exactly, so no line keeps its full rate. For example, a B2C deal where you strongly originated (${pct(CFG.strongOriginationB2cBp)}), closed (${pct(CFG.closingB2cBp)}), and delivered at ${pct(CFG.deliveryPercentBp)} adds to ${pct(strongSumB2c)}, which is above the ${pct(CFG.capB2cBp)} B2C cap, so the total is clamped to ${pct(CFG.capB2cBp)} and each of the three lines shrinks by the same proportion to fit.</p>
<p><strong>The two bonuses, and how they add.</strong> Beyond the per function rates, two bonuses can apply once you qualify. The growth bonus pays ${pct(CFG.growthBonusBp)} to a Tier 2 or Tier 3 partner who opens, closes, and collects ${CFG.growthBonusOrgThreshold} or more new B2B organizations within a rolling ${CFG.tierQualifyingWindowMonths} months, and it is paid inside the per deal cap. The Tier 3 focus bonus is different, and it is the only earning that can pay above the base cap. On a deal inside your active Tier 3 focus it starts at ${pct(CFG.focusBonusStartBp)} in your first year holding the focus and rises by ${pct(CFG.focusBonusAnnualIncrementBp)} for each additional full year you hold it continuously, up to a ceiling of ${pct(CFG.focusBonusCeilingBp)}, and it applies to seats you personally sourced or actively manage in the focus. That is why an eligible Tier 3 focus deal can rise gradually to ${pct(CFG.tier3FocusHardCeilingBp)}.</p>
<p><strong>The origination override on renewals.</strong> When you originate and open a B2B account, your earning on it does not end at the first deal. For the first ${CFG.originationWindowMonths} months after that account first closes, a same scope renewal on it also pays an origination override worth ${pct(CFG.overrideShareBp)} of the rate the account was originally opened at, on top of the renewal's own commission. Here is the key point on who receives it: the override is credited to the account's opener, the partner who originated it, not to whoever happens to record the renewal. Two conditions gate it and both must hold: the renewal falls inside that ${CFG.originationWindowMonths} month window measured from the account opening, and the opener still holds Active Status by actively supporting the account. Miss the window or let the opener go inactive and the override pays nothing. To make it concrete, an account opened at the strong B2B rate of ${pct(CFG.strongOriginationB2bBp)} pays a same scope renewal override of ${pct(overrideOnStrongB2b)} of that renewal's Net Receipts. Like every function, the override is one line on the renewal deal and sits inside that deal's single cap.</p><p><strong>When more than one partner works a deal.</strong> A deal can carry several functions performed by different people, and each line is credited to the partner who actually performed it, so an introducer, an originator, a closer, and a delivery partner can each be paid on the same deal. Where a single contribution is genuinely shared, the company can split that one function between partners by a set weight, recorded with a reason. However the credit is divided among partners, the deal total is still clamped to the same cap, so sharing a deal never breaks the ceiling.</p>
