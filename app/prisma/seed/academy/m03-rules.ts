...
// A B2C example that intentionally EXCEEDS the cap (once the B2C strong rate is
// unlocked) so the scale-down to fit is visible, config-sourced so it never drifts.
const strongSumB2c = CFG.strongOriginationB2cBp + CFG.closingB2cBp + CFG.deliveryPercentMaxBp;
...
<p>Commission is earned by function and calculated on net receipts actually received and cleared. The unit of sale is the seat, meaning one enrolled professional, sold either as a business to consumer charter (an individual enrolls) or a business to business engagement (an organization buys seats). Stronger origination and closing pay more than a basic introduction, delivery or coaching pays a fixed fee or small approved percentage, and there is a cap per deal. The exact figures live in the official terms. The logic to remember: you are paid on cleared money, for the function you performed, up to a cap, never simply because a conversation happened.</p><div class="callout callout-info"><p><strong>The current rates, by function.</strong> These are the current published rates. They can change, so always treat your Panel and the current published terms as the source of truth, not a number you memorized.</p></div><ul><li>Basic Introduction: ${pct(CFG.basicIntroductionBp)} on B2C and B2B, paid only when the contact you introduced becomes a closed, paid deal. An introduction does not register or protect the account for you, which is why origination pays more.</li><li>Qualified Origination: ${pct(CFG.qualifiedOriginationB2cBp)} on B2C, ${pct(CFG.qualifiedOriginationB2bBp)} on B2B.</li><li>Strong Origination: ${pct(CFG.strongOriginationB2cBp)} on B2C, ${pct(CFG.strongOriginationB2bBp)} on B2B.</li><li>Closing: ${pct(CFG.closingB2cBp)} on B2C, ${pct(CFG.closingB2bBp)} on B2B.</li><li>Delivery or Coaching: a fixed fee, or ${pct(CFG.deliveryPercentMinBp)} to ${pct(CFG.deliveryPercentMaxBp)} of Net Receipts. You do not set this rate yourself; the company approves it on the Panel and it stays inside that band.</li></ul><p><strong>Qualified or strong, and who decides.</strong> You never set your own rate or decide whether an origination counts as qualified or strong. The company classifies each origination and records it on the Panel, so you are not guessing. On a B2B Engagement both the qualified and the strong origination rate are available from the start. On a B2C Charter the strong rate unlocks only after you have sold and collected ${CFG.strongOriginationUnlockSeats} paid seats, or the company grants a Panel Confirmation. Until then a B2C origination you bring is paid at the qualified ${pct(CFG.qualifiedOriginationB2cBp)}, not the strong ${pct(CFG.strongOriginationB2cBp)}, so nothing on your statement is a surprise.</p><p><strong>The cap, and how functions add up.</strong> Total partner compensation on any one deal is capped: ${pct(CFG.capB2cBp)} of Net Receipts on B2C and ${pct(CFG.capB2bBp)} on B2B, and an eligible Tier 3 focus account can rise gradually to ${pct(CFG.tier3FocusHardCeilingBp)}. When you perform more than one function on the same deal, the rates add up, and the total is then clamped to that cap. Two plain examples. First, on a B2C deal where you qualified the opportunity (${pct(CFG.qualifiedOriginationB2cBp)}), closed it (${pct(CFG.closingB2cBp)}), and were approved to deliver at ${pct(CFG.deliveryPercentMaxBp)}, the rates add to ${pct(sumB2c)}. The B2C cap is ${pct(CFG.capB2cBp)}, so you are paid ${pct(paidB2c)} of Net Receipts, and any total above the cap is clamped to it. Second, on a B2B deal where you strongly originated (${pct(CFG.strongOriginationB2bBp)}), closed (${pct(CFG.closingB2bBp)}), and delivered at ${pct(CFG.deliveryPercentMaxBp)}, the rates add to ${pct(sumB2b)}. The B2B cap is ${pct(CFG.capB2bBp)}, so you are paid ${pct(paidB2b)}. For an ordinary deal this cap is an absolute ceiling that no extra function and no growth bonus can push past; the one exception is a Tier 3 focus deal, where the focus bonus can lift the ceiling gradually to ${pct(CFG.tier3FocusHardCeilingBp)}, explained next. When a stacked total does go over the cap, the percentage lines are scaled down in proportion to fit exactly, so no line keeps its full rate. For example, once your B2C strong rate is unlocked, a B2C deal where you strongly originated (${pct(CFG.strongOriginationB2cBp)}), closed (${pct(CFG.closingB2cBp)}), and delivered at ${pct(CFG.deliveryPercentMaxBp)} adds to ${pct(strongSumB2c)}, which is above the ${pct(CFG.capB2cBp)} B2C cap, so the total is clamped to ${pct(CFG.capB2cBp)} and each of the three lines shrinks by the same proportion to fit.</p>
...
<p><strong>The origination override on renewals.</strong> When you originate and open a B2B account, your earning on it does not end at the first deal. For the first ${CFG.originationWindowMonths} months after that account first closes, a same scope renewal on it also pays you an origination override worth ${pct(CFG.overrideShareBp)} of the rate the account was originally opened at, on top of the renewal's own commission. Two conditions gate it and both must hold: the renewal falls inside that ${CFG.originationWindowMonths} month window measured from the account opening, and you still hold Active Status by actively supporting the account. Miss the window or go inactive and the override pays nothing. A genuinely new engagement, or an expansion that adds at least ${CFG.majorNewEngagementMinSeats} paid seats, counts as a fresh opening and starts its own ${CFG.originationWindowMonths} month window. To make it concrete, an account opened at the strong B2B rate of ${pct(CFG.strongOriginationB2bBp)} pays a same scope renewal override of ${pct(overrideOnStrongB2b)} of that renewal's Net Receipts. Like every function, the override is one line on the renewal deal and sits inside that deal's cap. In short, opening a B2B account and staying active is worth a share of that account's renewals, not just its first sale.</p>
