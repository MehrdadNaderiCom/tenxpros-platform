...
const OUT = process.env.SHOT_OUT ?? "../docs/design/certification-rebuild-round-1";
const NAV_OFFSET = 80;
...
async function run(page, label) {
  await page.goto(`${BASE}/certification`, { waitUntil: "networkidle" });
  await settle(page);

  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(200);
  await page.screenshot({ path: `${OUT}/certification-${label}-top.png`, fullPage: false });

  await scrollToText(page, "The eight review criteria.");
  await page.screenshot({ path: `${OUT}/certification-${label}-criteria.png`, fullPage: false });

  if (label === "desktop") {
    await scrollToText(page, "Three outcomes. One standard.");
    await page.screenshot({ path: `${OUT}/certification-${label}-outcomes.png`, fullPage: false });
  }

  await scrollToText(page, "The credential has work behind it.");
  await page.screenshot({ path: `${OUT}/certification-${label}-proof.png`, fullPage: false });

  if (label === "desktop") {
    await scrollToText(page, "How verification works.");
    await page.screenshot({ path: `${OUT}/certification-${label}-verification.png`, fullPage: false });
  }

  await page.evaluate(() => window.scrollTo(0, document.documentElement.scrollHeight));
  await page.waitForTimeout(300);
  await page.screenshot({ path: `${OUT}/certification-${label}-bottom.png`, fullPage: false });
  ...
  if (label === "mobile") {
    ...
    await page.screenshot({ path: `${OUT}/certification-${label}-menu-open.png`, fullPage: false });
...
async function main() {
  ...
  for (const [label, r] of Object.entries({ desktop, mobile })) {
    ...
    for (const s of m.sections) console.log(`  ${s.found ? "✓" : "✗"} ${s.found ? `y=${s.absTop} vis=${s.visible} inDoc=${s.withinDoc}` : "MISSING"}  ${s.text.slice(0, 50)}`);
  }
  console.log(`\nwritten to ${OUT}`);
}
